...
from packages.core.constants import RISK_WEIGHTS
...
def _vector_search(
    *,
    ...
    # Import embed helpers - these stay in apps/api/embed for now
    ...
    if ids:
        ...
        for cid, score in fused_by_id.items():
            ...
            if isinstance(risk_tags, list):
                for t in risk_tags:
                    k_tag = str(t or "").strip()
                    if k_tag:
                        risk_score += int(RISK_WEIGHTS.get(k_tag, 0))
            items.append(
                {
                    "chunk_id": cid,
                    "excerpt": c["excerpt"],
