"""Command execution: mock and real runners.

Migrated from apps/api/runner/cmd.py — logic preserved exactly.
"""
...
def _mock_fixture_path(scenario: str) -> Path:
    ...
    # Try packages-relative path first, then apps/api path
    pkg_root = Path(__file__).resolve().parents[2]
    api_fixtures = pkg_root / "apps" / "api" / "tests" / "fixtures"
    if api_fixtures.exists():
        return api_fixtures / f"mock_{scenario}.json"
